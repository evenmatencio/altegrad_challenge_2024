"Define the classes to manipulate graphs and text (separately and jointly)."
...
class GraphTextDataset(Dataset):
    ...
    It has the following attributes:
        ## TEXT RELATED
        - input_ids (shape = [n_batch, 256]): the tokens ids corresponding to the
            chemical description of the graph. They are computed using the provided tokenizer,
            that should be specific to the TextEncoder we want to use. 
        - attention_mask (shape = [n_batch, 256]): the attention mask to be used by the
            TextEncoder model (DistillBert for instance)
        ## GRAPH RELATED
        - batch (shape=[tot_n_nodes]): the index of the graph to which each node belongs
            within the batch 
        - x (shape=[tot_n_nodes, 300]): the node features of all the nodes of the big disconnected
            graph modelling the batch (or equivalently of all the nodes of the different graphs
            in the batch)
        - edge_index (shape)=[2, tot_n_edges]): the edges index of the big disconnected graph
            modelling the batch
    """
    def __init__(self, root, gt, split, tokenizer=None,
                 transform=None, pre_transform=None, nrows=None):
        self.root = root
        self.gt = gt
        self.split = split
        self.tokenizer = tokenizer
        self.description = pd.read_csv(os.path.join(self.root, split+'.tsv'),
...
class GraphDataset(Dataset):
    def __init__(self, root, gt, split, transform=None, pre_transform=None, nrows=None):
        ...
        self.gt = gt
        self.split = split
        self.description = pd.read_csv(os.path.join(self.root, split+'.txt'),
                                       sep='\t', header=None, nrows=nrows)
